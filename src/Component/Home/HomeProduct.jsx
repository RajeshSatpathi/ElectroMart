...
function HomeProduct({ categoryName }) {
    const [data, setdata] = useState(products)
    useEffect(() => {
        if (!categoryName) {
            setdata(products)
        } else {
            const filterProduct = products.filter((item) => item.category === categoryName);
    ...
    const handleaddToCart = (product) => {
        dispatch(addToCart({
            ...
            name: product.name,
            price: product.price,
            quantity: 1, // default 1, can be dynamicm
        }))
                        ...
                        <div className='w-80 rounded mt-10 bg-white p-3 cursor-pointer'>
                            <div>
                                <div className=' flex justify-center items-center gap-4 '>
                                    ...
                                    <FaCartShopping size={25} color="white" className='bg-pink-500 rounded-full p-1.5' onClick={() => handleaddToCart(item)} />
